		...
		if(login_idx != null) { 
			...
			for(int i = 0; i < rList.size(); i++) {
				...
				for(int j = 0; j < sList.size(); j++) {
					...
					String str = String.format("{'reservation_idx':'%d', 'store_idx':'%d', 'staff_name':'%s', 'staff_grade':'%s', 'cal_day':'%s', 'getTime':'%s', 'surgery_name':'%s', 'complete':'%d'}", rList.get(i).getReservation_idx(), rList.get(i).getStore_idx(), sList.get(j).getName(), sList.get(j).getGrade(), rList.get(i).getCal_day(), rList.get(i).getGetTime(), rList.get(i).getSurgery_name(), rList.get(i).getComplete());
					arr += str;    
